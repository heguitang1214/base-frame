...
//        // "two");
//        String sqlParserForSelect = sqlParserForDelete(
//                "delete from persp where id in (select p.user_id from per p left join test a on(a.id = b.id) " +
//                        "inner join test t on(p.id = t.id) where t=1)",
//                "xxm");
